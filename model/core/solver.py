...
class Solver(PytreeNode, ABC):
    ...
    def RK4(state, grid, params, forcing_spectrum, ml_forcing=None, dt=None):
        ...
        dtt = params['dt'] if dt is None else dt

        rhs1, key = Solver.rhs(qh, k_noise, grid, params, forcing_spectrum, ml_forcing)
        rhs2, key = Solver.rhs(qh + 0.5 * dtt * rhs1, k_noise, grid, params, forcing_spectrum, ml_forcing)
        rhs3, key = Solver.rhs(qh + 0.5 * dtt * rhs2, k_noise, grid, params, forcing_spectrum, ml_forcing)
        rhs4, key = Solver.rhs(qh + dtt * rhs3, k_noise, grid, params, forcing_spectrum, ml_forcing)
        qh_new = qh + (dtt/6.0)*(rhs1 + 2*rhs2 + 2*rhs3 + rhs4)
        return (qh_new, key)
    
    @staticmethod
    @jax.jit
    def rhs(qh, key, grid, params, forcing_spectrum, ml_forcing=None):
        ...
        # Compute derivatives in physical space via spectral method using spectral qh
        dqdx = irfftn(1j * grid.KX * qh, axes=(-2,-1), norm='ortho').real
        dqdy = irfftn(1j * grid.KY * qh, axes=(-2,-1), norm='ortho').real
        ...
        if forcing_spectrum.shape != qh.shape:
            ...
            forcing_spec = forcing_spectrum

        forcing = forcing_h * forcing_spec
        forcing = Solver.dealias(forcing, grid, s=8)

        # --- ml forcing term (spectral) ---
        if ml_forcing is not None:
            ml_h = ml_forcing(qh, key_noise, grid, params)
            # assume ml_h is spectral and same shape as qh; apply dealias and add
            ml_h = Solver.dealias(ml_h, grid, s=8)
            forcing = forcing + ml_h

        rhs =  nonlinear + beta_term + forcing # forcing includes stochastic + ml
